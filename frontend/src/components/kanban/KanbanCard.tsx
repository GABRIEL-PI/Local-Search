    ...
    <Draggable draggableId={String(lead.id)} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`bg-surface rounded-lg border p-3 cursor-grab active:cursor-grabbing transition-shadow ${
            snapshot.isDragging ? 'shadow-lg rotate-1 border-blue-500/30' : 'shadow-sm border-border hover:shadow-md'
          }`}
        >
          <div className="flex items-start justify-between gap-2 mb-2">
            <Link
              ...
              className="text-sm font-medium text-fg hover:text-blue-600 dark:text-blue-400 line-clamp-2 leading-tight"
            >
              {lead.nome}
            </Link>
          ...
          {lead.categoria && (
            <p className="text-xs text-fg-subtle mb-2">{lead.categoria}</p>
          )}
