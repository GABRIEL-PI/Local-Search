  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-24">
        ...
        <span className="ml-2 text-fg-subtle">Carregando fila...</span>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-fg">Fila de Aprovação</h1>
          <p className="text-sm text-fg-subtle mt-1">Revise e aprove as propostas geradas pela IA</p>
        </div>
      ...
      {items.length === 0 ? (
        <Card>
          <CardContent className="py-24 text-center">
            <InboxIcon className="w-16 h-16 text-fg-faint mx-auto mb-4" />
            <h2 className="text-xl font-semibold text-fg">Fila vazia</h2>
            <p className="text-fg-faint mt-2 max-w-md mx-auto">
              ...
              <CardContent className="p-4 flex-1">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    {item.lead?.categoria && (
                      <p className="text-xs text-fg-subtle mt-0.5">{item.lead.categoria}</p>
                    )}
                    {item.lead?.cidade && (
                      <p className="text-xs text-fg-faint">{item.lead.cidade}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1 bg-violet-500/10 text-violet-600 dark:text-violet-400 px-2 py-1 rounded-full text-xs font-medium">
                ...
                {item.proposal.argumento_venda && (
                  <p className="text-xs text-fg-muted line-clamp-3 mb-3 p-2 bg-bg rounded-lg">
                    {item.proposal.argumento_venda}
                  </p>
                )}
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div className="bg-blue-500/10 rounded-lg p-2 text-center">
                    <p className="text-xs text-fg-subtle">Setup</p>
                    <p className="text-sm font-bold text-blue-700 dark:text-blue-300">
                      {formatCurrency(item.proposal.preco_sugerido)}
                    </p>
                  </div>
                  <div className="bg-emerald-500/10 rounded-lg p-2 text-center">
                    <p className="text-xs text-fg-subtle">Mensal</p>
                ...
                {item.proposal.mensagem_formal && (
                  <div className="bg-bg rounded-lg p-3 mb-3">
                    <div className="flex items-center gap-1 mb-1">
                      <MessageCircle className="w-3 h-3 text-fg-faint" />
                      <span className="text-xs text-fg-faint">Mensagem gerada</span>
                    </div>
                    <p className="text-xs text-fg line-clamp-2">
                      {item.proposal.mensagem_formal}
                    </p>
                  </div>
                )}

                <p className="text-xs text-fg-faint">{formatDate(item.proposal.criado_em)}</p>
              </CardContent>
      ...
      >
        {selectedProposal && (
          <div className="p-6 space-y-6">
            {/* Argument */}
            <div>
              <h4 className="text-sm font-semibold text-fg mb-2">Argumento de Venda</h4>
              <div className="prose prose-sm max-w-none text-fg bg-bg rounded-xl p-4 text-sm leading-relaxed whitespace-pre-wrap">
            ...
            {/* Messages */}
            <div>
              <h4 className="text-sm font-semibold text-fg mb-2">Mensagens para Envio</h4>
              ...
              <div className="bg-emerald-500/10 border border-green-100 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-emerald-700 dark:text-emerald-300">WhatsApp</span>
                </div>
            ...
            {/* Landing Page */}
            <div>
              <h4 className="text-sm font-semibold text-fg mb-2">Landing Page</h4>
